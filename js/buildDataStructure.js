...
job_player.buildDataStructure = function(player_instance) { 
    ...
    $.each(player_instance.interviews_json.talking_jobs.interview_responses.interview, function(interview_key, interview_value){
        ...
        $.each(interview_value.responses.response, function(response_key, response_value ) {
            ...
            playlist_object.now_image       = local_player_instance.image_prefix + interview_value.now_image['_src'];
            ...
            playlist_object.subtitles_url   = local_player_instance.subtitles_folder + response_value['_src'] + "_track0.xml";
            playlist_object.question_type   = local_player_instance.questions_json.talking_jobs.questions.question[response_key]['_type'];
            playlist_object.video           = local_player_instance.video_prefix + response_value['_src'] + ".mp4";
    ...
    $.each(player_instance.interviews_json.talking_jobs.interview_responses.interview, function(interview_key, interview_value ){
        local_player_instance.interviewees.push({interviewee: interview_value.job_title, id: interview_value['_id'], now_image: local_player_instance.image_prefix + interview_value.now_image['_src']});
    });
    ...
    local_player_instance.welcome_modal_image  = local_player_instance.image_prefix + local_player_instance.questions_json.talking_jobs.intro_copy.welcome_image;
